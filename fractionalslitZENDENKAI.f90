  module DATA01
  implicit none
  double precision theta,pi,f,epsilon0,mu0,omega,the0,theta0,ka,k,a,ab,krho
  ...
  complex(8),parameter::i=(0.d0,1.d0)
   ...
   program main
	 ...
	 use DATA04
	 implicit none
	 double precision the1,alpha,db,length,maxresult,hoge,distance
	 complex(8) Phi_1,Phi_2,Phi_i,Phi
   real(8) :: phi_t(3601)
   ...
   read(*,*) z_shisu
    ...
    ka=pi*length
	  a=ka/k
	  krho=2.d0*pi*distance
	  n=z_shisu+1
	  x_shisu_nu0=((nu-1.d0)/2.d0)
	  x_shisu_nu1=nu/2.d0


open(18,file='SlitTotal(MATLAB).TXT')
open(19,file='SlitTotal(Excel).TXT')

do the=-1799,1799,1
the1=dble(the)*0.1d0
theta=the1*(pi/180.00000000000000000000d0)
alpha=-k*dcos(theta)       !-kcosθ
Phi_i=cdexp(-i*krho*dcos(theta-theta0))
...
if(the.gt.-1800 .and. the.lt.(-1800+int(10.d0*the0)))then
  Phi=Phi_2(alpha)*k*dsin(dabs(theta))*cdexp(i*(krho-(pi/4.d0)))/dsqrt(krho)
  phi_t(the+1800)=cdabs(Phi+Phi_i)
  hoge=phi_t(the+1800)
 ...
 Phi=Phi_1(alpha)*k*dsin(dabs(theta))*cdexp(i*(krho-(pi/4.d0)))/dsqrt(krho)
  phi_t(the+1800)=cdabs(Phi+Phi_i)
  hoge=phi_t(the+1800)
else if(the.eq.(1800-int(10.d0*the0)))then
 phi_t(the+1800)=hoge
...
maxresult=MAXVAL(phi_t)
write(*,*) 'MAX=',maxresult

do the=1800,3599,1
write(18,*) 20.d0*log10(phi_t(the)/maxresult)
end do
...
do the=1,3599,1
write(19,*) 20.d0*log10(phi_t(the))
end do
